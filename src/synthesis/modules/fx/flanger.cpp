...
void Flanger::generate_buf() {
	...
	if (offset_mods) {
		for (size_t i{ 0 }; i < config::buffer_size; i++) {
			...
			const float_s delayed_signal_low{ memory_buffer.get(memory_buffer.size - config::buffer_size + i - effective_offset_floored) };
			const float_s delayed_signal_high{ memory_buffer.get(memory_buffer.size - config::buffer_size + i - effective_offset_floored - 1) };
